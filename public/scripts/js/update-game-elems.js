export function updatePaddleDirection(keys, leftPaddle, rightPaddle) {
    if (keys['w'] || keys['W']) {
        leftPaddle.dy = -7;
    }
...
export function update(gameState, ball, leftPaddle, rightPaddle, canvas, gameConfig) {
    leftPaddle.y += leftPaddle.dy;
    rightPaddle.y += rightPaddle.dy;
    leftPaddle.y = Math.max(Math.min(leftPaddle.y, canvas.height - gameConfig.paddleHeight), 0);
    rightPaddle.y = Math.max(Math.min(rightPaddle.y, canvas.height - gameConfig.paddleHeight), 0);
    ...
    function bounceOffPaddle(paddle, isLeft) {
    ...
    if (ball.x - ball.radius < leftPaddle.x + gameConfig.paddleWidth &&
        ball.y > leftPaddle.y &&
...
export function resetBall(ball, canvas, gameConfig) {
