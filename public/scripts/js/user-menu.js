...
export function updateUIforUserMenu(isLoggedIn) {
    const buttonCont = document.querySelector('.js-user-menu-button-container');
    if (!buttonCont)
        return;
    if (isLoggedIn) {
...
export function setUserMenuName() {
    ...
    const userMenuBtn = document.querySelector('.js-user-menu-button');
    userMenuBtn.textContent = userName;
}
export function handleToggleUserMenu(e) {
...
export function updateUIForAuthState(isLoggedIn) {
    ...
    if (!playConnectWalletBtns || !authBtns)
        return;
    if (isLoggedIn) {
